...
var checkCacheBefore = function( fn ) {
  return compose( function() {
    if ( !this.isRotCacheValid( this.gus.rotation )) this.updateCache( this.gus.rotation );
  }, fn );
}
...
GusMath.prototype.dot = function( vec1, vec2 ) {
  if ( vec1[0] === this.lastDot.vec1[0] && vec1[1] === this.lastDot.vec1[1] &&
       vec2[0] === this.lastDot.vec2[0] && vec2[1] === this.lastDot.vec2[1] ) return this.lastDot.returned;
